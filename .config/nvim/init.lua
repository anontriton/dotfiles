...
do
  ...
  -- Use 2-space indentation for C/C++ to match clang-format's default (LLVM) style
  vim.api.nvim_create_autocmd('FileType', {
    desc = 'Use 2-space indentation for C/C++',
    group = vim.api.nvim_create_augroup('custom-c-indent', { clear = true }),
    pattern = { 'c', 'cpp' },
    callback = function()
      vim.bo.tabstop = 2
      vim.bo.shiftwidth = 2
...
do
  ...
  for i, color in ipairs(foot_palette) do
    vim.g['terminal_color_' .. (i - 1)] = '#' .. color
  end

  -- No background, so foot's own (translucent) background shows through
  local function set_term_highlight() vim.api.nvim_set_hl(0, 'FootTermNormal', { fg = '#cdd6f4', bg = 'NONE' }) end
  set_term_highlight()
  vim.api.nvim_create_autocmd('ColorScheme', { callback = set_term_highlight })
  vim.api.nvim_create_autocmd('TermOpen', {
    callback = function() vim.wo[0][0].winhighlight = 'Normal:FootTermNormal,NormalNC:FootTermNormal' end,
  })
...
do
  ...
  require('conform').setup {
    ...
    -- Format-on-save is disabled; use <leader>f to format manually.
    default_format_opts = {
